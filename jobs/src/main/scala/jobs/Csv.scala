...
////    def fromRows(lines: UStream[Row]): Stream[E, A]
//    def fromRows(rows: UStream[Row]): Stream[E, A]
//  }
//
//  final private object ParseRows extends Parse[Nothing, Row] {
////    override def fromLines(lines: UStream[String]): Stream[Nothing, Row] =
////      lines.map(parseRow)
//    override def fromRows(rows: UStream[Row]): Stream[Nothing, Row] = rows
//  }
//
//  sealed trait ParseWithDecoder[-R, A] extends Any with Parse[RowFailure, A] {
//    def decoder: RowDecoder[R, A]
//  }
//
//  final private class ParseDecodeNoHeader[A] private[Csv] (
//    val decoder: RowDecoder.FromPositionOnly[A],
//  ) extends AnyVal
...
//    val decoder: RowDecoder.FromHeaderCtx[A],
//  ) extends AnyVal
//    with ParseWithDecoder[Has[HeaderCtx], A] {
//
//    /** Peels off the header row(s) and returns the [[HeaderCtx]] as well as
//      * another stream containing the rest of the rows of data.
//      */
//    def readHeader(rows: UStream[Csv.Row]): UIO[Option[(
//      HeaderCtx,
//      UStream[Csv.Row],
//    )]] = {
